...
void Linear_Chain::solve_self_consistent_Q(double Q_p, int zero_displacement, int zero_momentum) {
    arma::vec disp(dim);
    ...
    if (zero_displacement) {
        ...
        for(int i=0; i<Ns; i++) displacement(i) = 0;
    }

    //// Add random noise to Q and P with at the scale of randomness_level
    std::normal_distribution<double> rn;
    for(int i=0; i<Ns; i++) {
        displacement(i) += randomness_level * rn(rng);
        momentum(i) += randomness_level * rn(rng);
    }
...
void Linear_Chain::simulate_quench(unsigned long saved_steps, int pre_steps, double dt, int save_interval, int save_mid_interval, std::filesystem::path directory, int sim_num, int phase) {

    Hamiltonian = build_Hamiltonian(displacement);

    // THE following few lines will generate a random integer in [0, save_interval) in order to "offset" the save period with

    // Random number generator
    std::random_device rd_int;
    std::mt19937 gen(rd_int());
    std::uniform_int_distribution<> dis(0, save_interval-1);
    ...
    std::filesystem::path mid_save_path = directory / std::filesystem::path("mid_data_sim_" + std::to_string(sim_num) + ".dat");

    // Take (phase number + pre_steps) of steps first
    cout << "Integrating initial " << phase + pre_steps << " steps (phase + pre_steps)" << endl;
    for(unsigned long i=0; i<(unsigned long)(phase+pre_steps); i++) {
    ...
    // Variables used to determine how many steps to take before saving
    int pre_mid_interval = save_interval/2;
    int post_mid_interval = save_interval - pre_mid_interval;

    // Main integration loop
    // Integrates save_interval times and then saves the states. Does this for saved_steps times
    // If save_mid_interval is enabled, saves the mid-interval states into a separate dataset
    cout << "Beginning main integration loop" << endl;
    for(unsigned long i=0; i<saved_steps; i++) {

        if (i%100 == 0)
            cout << "time step:" << '\t' << i*save_interval << endl;

        // Integrate half of save_interval
        for(unsigned long j=0; j<(unsigned long)pre_mid_interval; j++) {
            integrate_EOM_RK4(dt);
        }

        // Save mid-interval states if enabled
        if (save_mid_interval){
            save_iteration(mid_save_path, Density_Mat, displacement, momentum);
        }

        // Integrate remaining half of save_interval
        for(unsigned long j=0; j<(unsigned long)post_mid_interval; j++) {
            integrate_EOM_RK4(dt);
        }
